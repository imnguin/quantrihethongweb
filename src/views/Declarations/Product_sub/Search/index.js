...
const Search = (props) => {
    ...
    const loadData = async (postData) => {
        ...
        const response = await dispatch(_fetchData(HOSTNAME, 'api/productsubgroup/search', postData));
        if (!response.iserror) {
    ...
    const onSubmit = (MLObject) => {
        let postData = {
            $or: [
                {
                    productsubgroupid: MLObject.keyword
                },
                {
                    productsubgroupname: MLObject.keyword
                }
            ]
        }
        loadData(postData);
        ...
        <>
            <SearchForm listColumn={SearchElement} layout='vertical' onSubmit={onSubmit} />
            {
                ...
                <DataGird
                    ...
                    isShowModalBtn={true}
                    TitleModal="nhóm sản phẩm"
                    listColumnModal={ElementList}
                    onSubmitModel={(values, action) => onSubmit({})}
                    hostName={HOSTNAME}
                    apiAdd='api/productsubgroup/add'
                    apiUpdate='api/productsubgroup/update'
                    apiDelete='api/productsubgroup/delete'
                    onSelectRowItem={(values) => onSubmit({})}
                    isExportTemplate={true}
                    fileTempalteData={fileTempalteData}
                    isImportExcel={true}
                    schema={schema}
                    apiImportExcel='api/productsubgroup/add'
