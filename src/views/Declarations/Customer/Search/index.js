...
const Search = (props) => {
    ...
    const loadData = async (postData) => {
        ...
        const response = await dispatch(_fetchData(HOSTNAME, 'api/customer/search', postData));
        if (!response.iserror) {
    ...
    return (
        <>
            <SearchForm listColumn={SearchElement} layout='vertical' onSubmit={onSubmit} />
            {
                ...
                <DataGird
                    pKey='customerid'
                    title='Danh sách sách khách hàng'
                    listColumn={columns}
                    dataSource={data}
